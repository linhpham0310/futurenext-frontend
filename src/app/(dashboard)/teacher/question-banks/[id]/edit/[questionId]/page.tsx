...
export default function EditQuestionItemPage() {
  const { id: bankId, questionId } = useParams();
  ...
  useEffect(() => {
    if (isTeacher && bankId && questionId) {
      teacherApi
        .getQuestionBank(bankId as string)
        .then((res) => {
          const item = res.data.items.find((q: any) => q.id === questionId);
          if (!item) {
  ...
  }, [bankId, questionId, isTeacher, router]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await teacherApi.updateQuestionItem(bankId as string, questionId as string, payload);
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Cập nhật câu hỏi</CardTitle>
        </CardHeader>
        <CardContent>
